...
| T-ID | PRD FR/NFR | Requirement (Atomic) | Component(s) | Test(s) | Status |
|------|------------|----------------------|--------------|---------|--------|
| T-HOOKS-001.1 | FR-16.8, FR-16.9, NFR-16.1.5 | Fix BSD awk deduplication (line 124: use `if ($0 in seen)` syntax) and remove broken co-occurrence filter (line 88: `grep -qE "co-occurs in [1-2] files" && continue`). | integrations/claude-code/plugin-maenifold/scripts/hooks.sh | Manual CLI: `hooks.sh session_start` produces non-empty CONCEPTS on macOS | Pending |
| T-HOOKS-001.2 | FR-16.10 | Add `IsDBNull` guard before `GetString` for community_id column in BuildContext SQLite reader. Return null community_id for unassigned concepts instead of crashing. | src/Utils/SqliteExtensions.cs | tests/Maenifold.Tests/CommunityDetectionTests.cs (extend existing) | Pending |
...
| T-HOOKS-001.4 | FR-16.1–16.7, FR-16.11, NFR-16.1.1–16.1.10 | Blue-team: Verify output format matches canonical example in PRD §4.8, token budget 150-350 with pruning (thread index first), community diversity ≥ 3 clusters when graph supports it, BSD compatibility on macOS, graceful fallback when no thinking sessions exist, total execution < 5s, ≤ 8 CLI calls, thread index capped at 5, no includeContent=true calls, WikiLink validation filters malformed concepts. | integrations/claude-code/plugin-maenifold/scripts/hooks.sh | Blue-team ConfessionReport | Pending |
| T-HOOKS-001.5 | Security | Red-team: Adversarial review — CLI injection via malicious concept names, timeout handling under slow/hanging CLI, error propagation to JSON output, jq injection via crafted BuildContext output, traceability audit. | All changed files | Red-team ConfessionReport | Pending |
